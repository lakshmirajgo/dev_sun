...
$page_buttom = get_pages_view('reserve_step2_buttom');
$company_info = get_company_info(); 
if ($_SESSION['auth'] != '1') {
header ("Location: create_account.php?redirect=reserve_step2.php");
};
...
if (isset($_POST)) {
 foreach($_POST as $key => $val)
    $_SESSION[$key] = $val;
			...
			$_SESSION['$myprice'] = $one_way['price_value']; 
			}
			if ($_SESSION['trip_type'] == '2') {
			$_SESSION['$myprice'] = $round_trip['price_value']; 
			}
			if ($_SESSION['trip_type'] > 2) {
			$_SESSION['$myprice'] = $transfer['price_value']; 
			}
...
window.location = "https://www.sunstatelimo.com/new3/reserve_step2.php";
}
-->
</script>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
      	<tr>
           ...
           <img src="images/secure_site.jpg" border="0" alt="THIS IS A SECURE SITE" title="THIS IS A SECURE SITE" />
            ...
            <td align="right" class="ot"><input src="images/continue.png" border="0" type="image" onclick="if (validateDate(reserve.date1.value, valDateFmt(reserve.datefmt),valDateRng(reserve.daterng))) {} else {alert('You can only reserve online if your arrival is <?php echo $company_info['minimum_time']*24; ?>hrs prior.\n\nPlease call 407-601-7900 to reserve over the phone'); return false;}; if (validateDate(reserve.date2.value, valDateFmt(reserve.datefmt),valDateRng(reserve.daterng))) {} else {alert('You can only reserve online if your arrival is <?php echo $company_info['minimum_time']*24; ?>hrs prior.\n\nPlease call 407-601-7900 to reserve over the phone'); return false;}; if (validateDate(reserve.date3.value, valDateFmt(reserve.datefmt),valDateRng(reserve.daterng))) {} else {alert('You
can only reserve online if your arrival is <?php echo $company_info['minimum_time']*24; ?>hrs prior.\n\nPlease call 407-601-7900 to reserve over the phone'); return false;}">
            </td>
        </tr>
      </table>  
    </div>
