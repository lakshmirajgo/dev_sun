<?php
	...
	function get_all_reservations(){
		global $db;
			$get_all_reservations_sql = "SELECT * FROM reservations ORDER BY id DESC LIMIT 30";
	...
	{	
        ...
        $sql="SELECT r.id, rd.date FROM reservations r INNER JOIN reservation_details rd ON r.id=rd.reservation_id WHERE r.status!='11' AND r.status!='7' AND rd.date=CURDATE()";
		$res=$db->select($sql);
		...
		while($info = $db->get_row($res,'MYSQL_ASSOC'))
		{
            $data[]=$info;
        }
	...
	$client_id = $_POST['client_id'];
		...
		$edit_reservations_sql = "UPDATE reservations SET client_id='".$client_id."', vehicle_id='".$_POST['vehicle_id']."', passenger_count='".$_POST['passenger_count']."', child_carseat='".$_POST['child_carseat']."', booster_seat='".$_POST['child_boosterseat']."', trip_type='".$_POST['trip_type']."', first_name='".
        ...
        $_POST['phone_number']."', cellphone='".
        $_POST['cellphone']."', first_name_billing='".
        $_POST['first_name_billing']."',last_name_billing='".
        $_POST['last_name_billing']."', city_billing='".
        $_POST['city_billing']."', state_billing='".
        $_POST['state_billing']."', zip_billing='".
        $_POST['zip_billing']."', country_billing='".
        $_POST['country_billing']."', card_number='".
        $_POST['card_number']."', card_type='".
        $_POST['card_type']."', exp_date='".$exp_date."', status='".$_POST['status']."', customer_comments='".
        $_POST['customer_comments']."', admin_comments='".
        $_POST['admin_comments']."', paying_cash='".$_POST['paying_cash']."', store_stop='".$_POST['store_stop']."', total_amount='".$_POST['total_amount']."', payment_status='".$_POST['payment_status']."', payment_date='$payment_date', approval_code='".$_POST['approval_code']."' where id='$id'";
		...
		$add_reservation_details_sql = "INSERT INTO reservation_details (`reservation_id`, `time`, `airline`, `flight_number`, `date`, `from`, `to`, `transfer_type`, `pickup_time`, `pickup_date`, `drivers_id`, `added_by_admin`) VALUES ('".$id."', '".$time[$count]."', '".$_POST['airline'.$count.'']."', '".$_POST['flight_number'.$count.'']."', '".$date[$count]."', '".$_POST['from'.$count.'']."', '".$_POST['to'.$count.'']."', '', '".$time_admin[$count]."', '".$date_admin[$count]."', '".$_POST['drivers_id'.$count.'']."', 1)";
	...
	//For Current Customers END
		...
		$add_reservation_details_sql = "INSERT INTO reservation_details (`reservation_id`, `time`, `airline`, `flight_number`, `date`, `from`, `to`, `transfer_type`, `pickup_time`, `pickup_date`, `added_by_admin`) VALUES ('".$res_id."', '".$time[$count]."', '".$_POST['airline'.$count.'']."', '".$_POST['flight_number'.$count.'']."', '".$date[$count]."', '".$_POST['from'.$count.'']."', '".$_POST['to'.$count.'']."', '', '".$time_admin[$count]."', '".$date_admin[$count]."', 1)";
	...
	function get_all_reservations_with_pages(){
		global $db;
			...
			// Make the links to other pages, if necessary.
				...
				// BEGIN DISPLAY TOTAL PAGE
					echo 'Page <strong>' . $current_page . '</strong> of <strong>' . $num_pages . '</strong> &nbsp; &nbsp; ';
				// END DISPLAY TOTAL PAGE
				
				//If it's not the first page, make a`Previous button.
				if ($current_page != 1) {
					// BEGIN DISPLAY FIRST PAGE LINK
					echo ' |&lt; <a href="reservation_manager.php?orderby=' .$_GET['orderby']. '&sort=' .$_GET['sort']. '&pages=' .$display. '&s=0&np=' . $num_pages . '&search_by_sql='.$_GET['search_by_sql'].'" class="bodytxt">first</a> ';
					// END DISPLAY FIRST PAGE LINK
				...
				for ($i = 1; $i <= $num_pages; $i++) {	
						// BEGIN LIMIT PAGE COUNT LINKS
						$before_cp = ceil(((int)$current_page) - 7);
						$after_cp = ceil(((int)$current_page) + 7);
						if($i <= $before_cp || $i >= $after_cp) continue;
						// END LIMIT PAGE COUNT LINKS
				if ($i !=$current_page) {
				...
				echo '<a href="reservation_manager.php?orderby=' .$_GET['orderby']. '&sort=' .$_GET['sort']. '&pages=' .$display. '&s=' . ($start + $display) . '&np=' . $num_pages . '&search_by_sql='.$_GET['search_by_sql'].'" class="bodytxt">next</a> &gt;&gt;';
					// BEGIN DISPLAY LAST PAGE LINK
					echo ' <a href="reservation_manager.php?orderby=' .$_GET['orderby']. '&sort=' .$_GET['sort']. '&pages=' .$display. '&s=' . ($display * ($num_pages-1)) . '&np=' . $num_pages . '&search_by_sql='.$_GET['search_by_sql'].'" class="bodytxt">last</a> &gt;|';
					// END DISPLAY LAST PAGE LINK
				}
				echo '</span></td>';
				echo '</tr></table>';
				echo '</form>';
	...
	function get_report_reservations($from, $to, $vehicle_id, $status_id, $trip_type){
		...
		if (!empty($trip_type)) {
			if (strtolower($trip_type) == 'sanford') {
				$searchby_sql4 = " AND (rd.`from` = 422 OR rd.`to` = 422)";
			}
			else {
				$searchby_sql4 = " AND r.trip_type='$trip_type'";
			}
		}
		if ($from == $to) {
			$searchby_sql3 = " rd.pickup_date LIKE '$to%'";
		} 
		else {
			if ($from =='--' || $to =='--') {
				if ($from =='--') {
				...
				$searchby_sql3 = " rd.pickup_date LIKE '$from%'";
				}
			} else {
			$searchby_sql3 = " rd.pickup_date BETWEEN '$from' AND '$to'";
			};
		};
	...
	//Get Report Reservations - Last Version
	function get_report_reservations_sanford($from, $to, $vehicle_id, $status_id, $trip_type) {
		global $db;
		if (!empty($vehicle_id)) {
		$searchby_sql = " AND vehicle_id='$vehicle_id'";
		}
...
&nbsp;                      </td>
	...
	function get_reservation_view($id){
        ...
        {
            $get_reservation_view_sql = "SELECT * FROM reservations where id='1178'";
            $db->select($get_reservation_view_sql);
            $result = $db->select($get_reservation_view_sql);
            $data = $db->get_row($result, 'MYSQL_ASSOC');
