<?php
	...
	function get_all_reservations_for_email(){
		global $db;
			...
			$get_all_reservations_for_email_sql = "SELECT distinct r.id, r.email, r.first_name, r.last_name, r.status FROM reservations r INNER JOIN reservation_details rd ON r.id=rd.reservation_id WHERE r.status !='11' or r.status !='7' ORDER BY r.id DESC";
	...
	function get_all_reservations_for_statuses(){
		global $db;
			...
			// Select where status != Credit card verification error OR Cancelled 	
			$get_all_reservations_for_statuses_sql = "SELECT distinct r.id, r.email, r.first_name, r.last_name, r.status FROM reservations r INNER JOIN reservation_details rd ON r.id=rd.reservation_id WHERE r.status !='11' or r.status !='7' ORDER BY r.id DESC";
	...
	//For Current Customers END
		...
		$time_admin = format_time_admin_mysql($time[$count], $_POST['from'.$count.''], $_POST['to'.$count.''], $date[$count]);
		//echo $time[$count];
		//echo $mytime = $time[$count] - 160;
		echo $time_admin;
		exit;
		//Pick up time calculation END
		
		$add_reservation_details_sql = "INSERT INTO reservation_details (`reservation_id`, `time`, `airline`, `flight_number`, `date`, `from`, `to`, `transfer_type`) VALUES ('".$res_id."', '".$time[$count]."', '".$_POST['airline'.$count.'']."', '".$_POST['flight_number'.$count.'']."', '".$date[$count]."', '".$_POST['from'.$count.'']."', '".$_POST['to'.$count.'']."', '')";
	...
	function get_all_reservations_with_pages(){
		global $db;
			...
			$search_by_sql = 'WHERE trip_type >= 14';
			};
			...
			$query = "SELECT id, client_id, vehicle_id, passenger_count, child_carseat, booster_seat, trip_type, first_name, last_name, address, address2, city, state, zip, country, email, first_name_billing, last_name_billing, address_billing, address2_billing, city_billing, state_billing, zip_billing, country_billing, total_amount, status, customer_comments, admin_comments, paying_cash, store_stop, ip_address, reservation_date FROM reservations $search_by_sql ORDER BY $orderby_sql LIMIT $start, $display";
	...
	function get_report_reservations($from, $to, $vehicle_id, $status_id, $trip_type){
		...
		$searchby_sql3 = " LIKE '$to%'";
			...
			$get_report_reservations_sql = "SELECT distinct r.id, r.vehicle_id, r.trip_type, r.total_amount, v.name, r.first_name, r.last_name, r.status, r.reservation_date FROM reservations r INNER JOIN vehicles v ON r.vehicle_id=v.id LEFT JOIN reservation_details rd ON r.id=rd.reservation_id WHERE rd.date $searchby_sql3 $searchby_sql $searchby_sql2 ORDER BY rd.date DESC";
			...
			if(!$result = $db->select($get_report_reservations_sql)){
				...
				while($data=$db->get_row($result, 'MYSQL_ASSOC'))
					//$report_reservations[] = $data['id'];
					//$report_reservations = array_unique($report_reservations);
	...
	function get_schedule(){
		global $db;
			...
			$order_by_sql = " rd.time ASC";
			} else {
				if ($_GET['order_by'] == 'time') {
					if ($_GET['sort'] == 'asc') {
					$order_by_sql = " rd.time ASC";
					} else {
					$order_by_sql = " rd.time DESC";
					}
				}
			...
			$query = "SELECT rd.id, rd.reservation_id, rd.time, rd.airline, rd.flight_number, rd.date, rd.store_stop, rd.from, rd.to, rd.transfer_type, r.client_id, r.vehicle_id, r.passenger_count, r.child_carseat, r.booster_seat, r.trip_type, r.first_name, r.last_name, r.address, r.address2, r.city, r.state, r.zip, r.country, r.email, r.first_name_billing, r.last_name_billing, r.address_billing, r.address2_billing, r.city_billing, r.state_billing, r.zip_billing, r.country_billing, r.total_amount, r.status, r.customer_comments, r.cellphone, r.admin_comments, r.paying_cash, r.store_stop, r.ip_address, t.num_legs FROM reservation_details rd INNER JOIN reservations r ON rd.reservation_id = r.id INNER JOIN trip_types t ON r.trip_type = t.id WHERE r.status !='11' AND rd.date = '$todays_date' ORDER BY $order_by_sql LIMIT $start, $display";
      ...
      <tbody><tr>
		...
		<table width="100%" cellpadding="5" cellspacing="0" border="1">
            ...
            <?php 	
					...
					$departure_value='';
					if(check_departure($value['from']) || check_arrival($value['from']) || check_departure($value['to']) || check_arrival($value['to'])) { if (check_arrival($value['from'])) { $bgcolor="#004eff"; } else { $bgcolor="#ff0000"; if($value['trip_type'] !='1') { $departure_value='yes'; }; }; ?><? } else { $bgcolor="#f49502"; };
					};
					echo '<tr>';
					?>
                    ...
                    <tr>
                	  ...
                	  <td bgcolor="#FFFFFF" colspan="4" class="ob" valign="top"><?php if ($value['paying_cash']) { ?>Please do not charge my credit card. I will be paying cash or traveler check upon arrival.<br /><?php } ?><?php if (!empty($value['customer_comments'])) { echo $value['customer_comments'].'<br />'; }; ?><?php if (!empty($value['cellphone'])) { echo '<strong>Cellphone:</strong> '.$value['cellphone']; }; ?></td>
            		</tr>
                    <? 
					} 
					?>
...
//--------------- TOMORROW SCHEDULE by RaSH  ---------------------------------
	...
	function get_schedule_tomorrow(){
		global $db;
			...
			$query = "SELECT rd.id, rd.reservation_id, rd.time, rd.airline, rd.flight_number, rd.date, rd.store_stop, rd.from, rd.to, rd.transfer_type, r.client_id, r.vehicle_id, r.passenger_count, r.child_carseat, r.booster_seat, r.trip_type, r.first_name, r.last_name, r.address, r.address2, r.city, r.state, r.zip, r.country, r.email, r.first_name_billing, r.last_name_billing, r.address_billing, r.address2_billing, r.city_billing, r.state_billing, r.zip_billing, r.country_billing, r.total_amount, r.status, r.customer_comments, r.admin_comments, r.paying_cash, r.store_stop, r.ip_address, t.num_legs FROM reservation_details rd INNER JOIN reservations r ON rd.reservation_id = r.id INNER JOIN trip_types t ON r.trip_type = t.id WHERE rd.date = '$tomorrow' ORDER BY $order_by_sql";
			...
			$query = "SELECT rd.id, rd.reservation_id, rd.time, rd.airline, rd.flight_number, rd.date, rd.store_stop, rd.from, rd.to, rd.transfer_type, r.client_id, r.vehicle_id, r.passenger_count, r.child_carseat, r.booster_seat, r.trip_type, r.first_name, r.last_name, r.address, r.address2, r.city, r.state, r.zip, r.country, r.email, r.first_name_billing, r.last_name_billing, r.cellphone, r.address_billing, r.address2_billing, r.city_billing, r.state_billing, r.zip_billing, r.country_billing, r.total_amount, r.status, r.customer_comments, r.admin_comments, r.paying_cash, r.store_stop, r.ip_address, t.num_legs FROM reservation_details rd INNER JOIN reservations r ON rd.reservation_id = r.id INNER JOIN trip_types t ON r.trip_type = t.id WHERE r.status !='11' AND rd.date = '$todays_date' ORDER BY $order_by_sql LIMIT $start, $display";
...
//--------- TOMORROW SCHEDULE FUNCTION END -----------------------------------	
	...
	function get_arrivals_with_pages(){
		global $db;
			...
			if (empty($_GET['orderby'])) {
			$orderby_sql = " rd.time ASC";
			} else {
				if ($_GET['orderby'] == 'time') {
					if ($_GET['sort'] == 'asc') {
					$orderby_sql = " rd.time ASC";
					} else {
					$orderby_sql = " rd.time DESC";
					}
				}
			...
			$todays_date = date('Y-m-d'); 
			$query = "SELECT rd.id, rd.reservation_id, rd.time, rd.airline, rd.flight_number, rd.date, rd.store_stop, rd.from, rd.to, rd.transfer_type, r.client_id, r.vehicle_id, r.passenger_count, r.child_carseat, r.booster_seat, r.trip_type, r.first_name, r.last_name, r.address, r.address2, r.city, r.state, r.zip, r.country, r.email, r.first_name_billing, r.last_name_billing, r.address_billing, r.address2_billing, r.city_billing, r.state_billing, r.zip_billing, r.country_billing, r.total_amount, r.status, r.customer_comments, r.admin_comments, r.paying_cash, r.store_stop, r.ip_address, t.num_legs FROM reservation_details rd INNER JOIN reservations r ON rd.reservation_id = r.id INNER JOIN trip_types t ON r.trip_type = t.id WHERE (rd.from='422' OR rd.from='421' OR rd.from='512') AND rd.date = '$todays_date' ORDER BY $orderby_sql";
	...
	function get_departures_with_pages(){
		global $db;
			...
			$todays_date = date('Y-m-d'); 
			$query = "SELECT rd.id, rd.reservation_id, rd.time, rd.airline, rd.flight_number, rd.date, rd.store_stop, rd.from, rd.to, rd.transfer_type, r.client_id, r.vehicle_id, r.passenger_count, r.child_carseat, r.booster_seat, r.trip_type, r.first_name, r.last_name, r.address, r.address2, r.city, r.state, r.zip, r.country, r.email, r.first_name_billing, r.last_name_billing, r.address_billing, r.address2_billing, r.city_billing, r.state_billing, r.zip_billing, r.country_billing, r.total_amount, r.status, r.customer_comments, r.admin_comments, r.paying_cash, r.store_stop, r.ip_address, t.num_legs FROM reservation_details rd INNER JOIN reservations r ON rd.reservation_id = r.id INNER JOIN trip_types t ON r.trip_type = t.id WHERE (rd.to='422' OR rd.to='421' OR rd.to='512') AND rd.date = '$todays_date' ORDER BY $orderby_sql";
	...
	function get_transfers_with_pages(){
		global $db;
			...
			$todays_date = date('Y-m-d'); 
			$query = "SELECT rd.id, rd.reservation_id, rd.time, rd.airline, rd.flight_number, rd.date, rd.store_stop, rd.from, rd.to, rd.transfer_type, r.client_id, r.vehicle_id, r.passenger_count, r.child_carseat, r.booster_seat, r.trip_type, r.first_name, r.last_name, r.address, r.address2, r.city, r.state, r.zip, r.country, r.email, r.first_name_billing, r.last_name_billing, r.address_billing, r.address2_billing, r.city_billing, r.state_billing, r.zip_billing, r.country_billing, r.total_amount, r.status, r.customer_comments, r.admin_comments, r.paying_cash, r.store_stop, r.ip_address, t.num_legs FROM reservation_details rd INNER JOIN reservations r ON rd.reservation_id = r.id INNER JOIN trip_types t ON r.trip_type = t.id WHERE (rd.from !='422' AND rd.from !='421' AND rd.from !='512' AND rd.to !='422' AND rd.to !='421' AND rd.to !='512') AND rd.date = '$todays_date' ORDER BY $orderby_sql";
			...
			$query = "SELECT rd.id, rd.reservation_id, rd.time, rd.airline, rd.flight_number, rd.date, rd.store_stop, rd.from, rd.to, rd.transfer_type, r.client_id, r.vehicle_id, r.passenger_count, r.child_carseat, r.booster_seat, r.trip_type, r.first_name, r.last_name, r.address, r.address2, r.city, r.state, r.zip, r.country, r.email, r.first_name_billing, r.last_name_billing, r.address_billing, r.address2_billing, r.city_billing, r.state_billing, r.zip_billing, r.country_billing, r.total_amount, r.status, r.customer_comments, r.cellphone, r.admin_comments, r.paying_cash, r.store_stop, r.ip_address, t.num_legs FROM reservation_details rd INNER JOIN reservations r ON rd.reservation_id = r.id INNER JOIN trip_types t ON r.trip_type = t.id WHERE r.status !='11' AND (rd.from !='422' AND rd.from !='421' AND rd.from !='512' AND rd.to !='422' AND rd.to !='421' AND rd.to !='512') AND rd.date = '$todays_date' ORDER BY $orderby_sql LIMIT $start, $display";
	...
	function get_future_reservations_with_pages(){
		global $db;
			...
			$orderby_sql = " rd.date ASC, rd.time ASC";
			} else {
			...
			if (!empty($_POST['from'])) {
			$todays_date = format_date_calendar($_POST['from']);
			} else {
			$todays_date = date('Y-m-d'); 
			}
			
			if (!empty($_POST['to'])) {
			$expiration_date = format_date_calendar($_POST['to']);
			} else {
			$expiration_time = strtotime("+30 days");
			$expiration_date = date("Y-m-d", $expiration_time);
			}
			//print_r($expiration_date);
			$query = "SELECT rd.id, rd.reservation_id, rd.time, rd.airline, rd.flight_number, rd.date, rd.store_stop, rd.from, rd.to, rd.transfer_type, r.client_id, r.vehicle_id, r.passenger_count, r.child_carseat, r.booster_seat, r.trip_type, r.first_name, r.last_name, r.address, r.address2, r.city, r.state, r.zip, r.country, r.email, r.first_name_billing, r.last_name_billing, r.address_billing, r.address2_billing, r.city_billing, r.state_billing, r.zip_billing, r.country_billing, r.total_amount, r.status, r.customer_comments, r.admin_comments, r.paying_cash, r.store_stop, r.ip_address, t.num_legs FROM reservation_details rd INNER JOIN reservations r ON rd.reservation_id = r.id INNER JOIN trip_types t ON r.trip_type = t.id WHERE (rd.date BETWEEN '$todays_date' AND '$expiration_date') ORDER BY $orderby_sql";
			//print_r($query);
			//exit;
			$query_result=mysql_query($query);
			$num_records=@mysql_num_rows($query_result);
			...
			$query = "SELECT rd.id, rd.reservation_id, rd.time, rd.airline, rd.flight_number, rd.date, rd.store_stop, rd.from, rd.to, rd.transfer_type, r.client_id, r.vehicle_id, r.passenger_count, r.child_carseat, r.booster_seat, r.trip_type, r.first_name, r.last_name, r.address, r.address2, r.city, r.state, r.zip, r.country, r.email, r.first_name_billing, r.last_name_billing, r.address_billing, r.address2_billing, r.city_billing, r.state_billing, r.zip_billing, r.country_billing, r.total_amount, r.status, r.customer_comments, r.cellphone, r.admin_comments, r.paying_cash, r.store_stop, r.ip_address, t.num_legs FROM reservation_details rd INNER JOIN reservations r ON rd.reservation_id = r.id INNER JOIN trip_types t ON r.trip_type = t.id WHERE r.status !='11' AND (rd.date BETWEEN '$todays_date' AND '$expiration_date') ORDER BY $orderby_sql LIMIT $start, $display";
...
&nbsp;                      </td>
                	  <td bgcolor="#FFFFFF" colspan="4" class="ob" valign="top"><?php if ($value['paying_cash']) { ?>Please do not charge my credit card. I will be paying cash or traveler check upon arrival.<br /><?php } ?><?php if (!empty($value['customer_comments'])) { echo $value['customer_comments'].'<br />'; }; ?><?php if (!empty($value['cellphone'])) { echo '<strong>Cellphone:</strong> '.$value['cellphone']; }; ?>&nbsp;<?php //echo "<strong>".format_time_admin_note($value['time'], $value['from'], $value['to'], $value['date'])."</strong>";  ?></td>
            		</tr>
                    <? 
					} 
					?>
