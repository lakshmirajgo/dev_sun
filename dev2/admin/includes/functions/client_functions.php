<?php
	...
	/**
	 *	fetch the most recent reservation with payment information in it
	 *	added 2013.10.08
	 */
	function get_client_recent_payment_info_blah($client_id) {
		global $db;
		$sql = "SELECT
					first_name_billing,
					last_name_billing,
				...
				ORDER BY id DESC
				LIMIT 1";
		if ( ! $res = $db->select($sql)) {
			//$_SESSION['notice'] = "Database Error. Please try again";
			return false;
		}
		else {
			$row = $db->get_row($res, 'MYSQL_ASSOC');
			$exp_date = str_replace('/', ' 01, ', $row['exp_date']);
			$row['ExpMonth'] = date('M', strtotime($exp_date));
			$row['ExpYear'] = date('Y', strtotime($exp_date));			
			return $row;
		}
	}
	...
	function get_all_clients(){
		global $db;
			$get_all_clients_sql = "SELECT * FROM clients ORDER BY last_name ASC";
	...
	global $db;
		$edit_clients_sql = "UPDATE clients SET first_name='".
            ...
            mysql_real_escape_string($_POST['country'])."', telephone='".
            mysql_real_escape_string($_POST['telephone'])."', telephone2='".
            mysql_real_escape_string($_POST['telephone2'])."', cellphone='".
            mysql_real_escape_string($_POST['cellphone'])."', fax='".
            mysql_real_escape_string($_POST['fax'])."', email='".
            mysql_real_escape_string($_POST['email'])."' where id='$id'";
	...
	$signup_date = date('Y-m-j, h-i-s');
        ...
        mysql_real_escape_string($_POST['telephone2'])."', '".
	...
	function get_all_clients_with_pages(){
      ...
      <tbody><tr>
		...
		<table width="820" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" style="margin-top: 10px;" class="bodytxt" align="center">
             <tr bgcolor="#646464" >
               		  <td width="37" align="center" class="ot1"><input type="checkbox" name="checkall" onclick="checkedAll(document.displayfrm.id);"></td>
                      ...
                      <td width="155" align="left" style="font-weight: bold; color:#FFFFFF" class="ot1">
                      <?php
                      if ($_GET['sort'] == 'asc' || $_GET['sort'] == '') {
					  ?>
                      ...
                      <td width="100" align="center" style="font-weight: bold; color:#FFFFFF" class="ot1">Reservations</td>
                  ...
                  <?php 	
					...
					?>
                      ...
                      <td width="155" align="left" class="ot1"><a href="mailto:<?php echo $value['email']; ?>" class="contact_link"><?php echo $value['email']; ?></a></td>
                      <td width="100" height="22" align="center" class="ot1"><?php
					   $clients_reservations = count_clients_reservations($value['id']); ?><a href="reservation_search.php?search_by=client_id&where=<?php echo $value['id']; ?>" class="bodytxt"><img src="images/find.png" border="0" title="View Reservations" alt="View Reservations" align="absmiddle" /></a> <a href="reservation_search.php?search_by=client_id&where=<?php echo $value['id']; ?>" class="bodytxt"><strong>(<?php echo count($clients_reservations); ?>)</strong></a></td>
                      <td width="60" height="22" align="center"><a href="client_manager.php?cAction=edit&id=<?php echo $value['id']; ?>" class="bodytxt" title="Click to Edit"><img src="images/edit.png" border="0" /></a>&nbsp;&nbsp;<a href="?cAction=delete&id=<?php echo $value['id']; ?>" class="bodytxt" title="Click to Delete"><img src="images/remove.png" border="0" onclick="return confirm('Are you sure you want to delete this client?\n\nNotice: deleted client cannot be restored')" /></a></td>
                      <td width="5"></td>
                    </tr>
                    <? 
					} 
