...
if (!empty($_POST['calculate_total'])){
	 ...
	 $report_reservations = get_report_reservations_sanford($from, $to, $vehicle_id, $status_id, $trip_type);
...
<?php include ("includes/common/menu.php");	?>
	...
	?>
    <form name="report" method="post" action="get_report_sanford.php" style="padding:0px; margin:0px;">
      ...
      <tbody><tr>
		...
		<table width="1210" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" style="margin-top: 10px;" class="bodytxt" align="center">
             <tr bgcolor="#646464" >
                  ...
                  <?php /*
                  <div align="right" style="padding-right:40px; padding-top:5px; padding-bottom:5px;">Total NET: <strong>$<?php
				  $total_net = $total - $sreptotal;
				   if (!empty($total_net)) { echo sprintf("%01.2f", $total_net); } else { echo "0.00"; };?></strong>
          ...
          <tr>
            <td align="left" valign="top"><table class="bodytxt" border="0" cellpadding="0" cellspacing="0" width="100%">
              <tbody>
              <tr>
                <td align="left" height="48" valign="middle" background="images/bottom_center_curve.jpg"><input name="calculate_total" type="submit" value="Get Report"> &nbsp;&nbsp;<strong>For Vehicle</strong> <select name="vehicle_id" size="1" class="bodytxt">
                <?php if (!empty($_POST['vehicle_id'])) {
				$vehicle_view = get_vehicles_view($_POST['vehicle_id']);
				echo '<option value="'.$_POST['vehicle_id'].'">'.$vehicle_view['name'].'</option>';
				}
				?>
                <option value="">All Vehicles</option>
               	<?php
				...
				?>
                </select> &nbsp;&nbsp;<strong>Status</strong> <select name="status_id" size="1" class="bodytxt">
                <?php if (!empty($_POST['status_id'])) {
				$status_view = get_statuses_view($_POST['status_id']);
				echo '<option value="'.$_POST['status_id'].'">'.$status_view['name'].'</option>';
				}
				?>
                <option value="">All Statuses</option>
               	<?php
				...
				?>
                </select> &nbsp;&nbsp;<strong>Trip Type</strong> <select name="trip_type" size="1" class="bodytxt">
                <?php if (!empty($_POST['trip_type'])) {
				$trip_type = get_trip_types_view($_POST['trip_type']); 
				echo '<option value="'.$_POST['trip_type'].'">'.$trip_type['name'].'</option>';
				}
				?>
