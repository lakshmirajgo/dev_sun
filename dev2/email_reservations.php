...
if(!isset($_GET['id'])) die('You must submit a reservation id as a GET parameter');

/** NOTE: THIS FILE WAS MODIFIED TO RE-GENERATE THE ORIGINAL EMAILS SENT TO THE CLIENT AND ADMIN. DO NOT USE UNLESS THATS WHAT YOU ARE TRYING TO DO **/
/** USE BY ENTERING THE RESERVATION ID AS A GET PARAMETER **/

$SEND_EMAIL_TO = isset($_GET['email']) ? $_GET['email'] : 'martin@imperialwebsolutions.net';
...
$reservation_view = get_reservation_view($id);	
...
Mobile Phone: '.$reservation_view['cellphone'].'<br />';
		  ...
		  Passengers: '.$reservation_view['passenger_count'].'<br />';
			 ...
			 Quick Grocery Stop: '.$reservation_view['store_stop'].'<br />';
			  ...
			  $reservation_details = get_all_reservation_details($id);
...
$message_sunstate_admin .= $message_sunstate;
	...
	"Content-type: text/html; charset=iso-8859-1\r\n".
    'X-Mailer: PHP/' . phpversion();
	mail($to, 'New Transportation Request', $message_sunstate_admin, $headers);
