<?php
	...
	$client_id = $_POST['client_id'];
		...
		//echo $_POST['from'.$count.''];
		//exit;
		//echo $time_admin[$count];
		//exit;
		
		$edit_reservation_details_sql = "UPDATE reservation_details SET time='".$time[$count]."', pickup_time='".$time_admin[$count]."', pickup_date='".$date_admin[$count]."', airline='".$_POST['airline'.$count.'']."', flight_number='".$_POST['flight_number'.$count.'']."', date='".$date[$count]."' WHERE id ='".$_POST['details_id'.$count.'']."'";
...
//--------- TOMORROW SCHEDULE FUNCTION END -----------------------------------	
	...
	function get_future_reservations_with_pages(){
		global $db;
			...
			if (isset($_POST['from'])) {
			$date_arr = explode("/", $_POST['from']);
			$todays_date = $date_arr[2] . "-".$date_arr[0] . "-".$date_arr[1];
			} else {
			$todays_date = date('Y-m-d'); 
			}
			
			if (isset($_POST['to'])) {
			$exp_arr = explode("/", $_POST['to']);
			$expiration_date = $exp_arr[2] . "-".$exp_arr[0] . "-".$exp_arr[1];
			} else {
